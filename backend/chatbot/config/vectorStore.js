...
class VectorStoreManager {
  ...
  async initialize() {
    ...
    try {
      ...
      try {
        ...
        console.log("📦 No existing vector store found, building new one...");
  ...
  async buildKnowledgeBase() {
    ...
    try {
      ...
      console.log(`✅ Knowledge base built: ${splitDocs.length} chunks indexed`);
    } catch (error) {
      ...
      if (typeof obj === 'string' && obj.length > 20) {
        ...
        obj.forEach((item, idx) => {
          if (typeof item === 'object') {
            ...
            documents.push(new Document({
              pageContent: item,
              metadata: { source, role, category: path[0] || 'general', path: path.join(' > ') }
            }));
          }
        });
  ...
  async createSampleKnowledge() {
    const sampleData = {
      "overview": "Welcome to our education loan platform. We help students secure loans for studying abroad.",
      "loan_process": {
        "description": "Our loan process is simple and fast. Complete your profile, upload documents, get matched with NBFCs, and receive approval within 7-14 days.",
        "steps": [
          "Create your account",
      ...
      "documents": {
        "academic": "We need your 10th, 12th marksheets, and degree certificates to verify your academic background.",
        "admission": "Admission letter from your university is required to process your loan application.",
        "kyc": "Aadhaar, PAN, and Passport are mandatory for identity verification.",
        "financial": "Bank statements and income proof help us assess your repayment capacity."
      },
      "faqs": [
        {
        ...
        },
        {
          ...
          "answer": "Interest rates range from 9.5% to 15% depending on your profile and the NBFC."
        }
      ]
    };
