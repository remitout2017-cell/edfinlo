  ...
  const fetchConsultantProfile = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/consultant/auth/me`,
        {
          headers: {
      ...
      try {
        ...
        if (response.data.success) {
          return {
            ...
            totalStudents:
              response.data.totalStudents || response.data.data?.length || 0,
    ...
    async (analysisId) => {
      ...
      try {
        const response = await axios.delete(
          `${API_BASE_URL}/api/consultant/loan-analysis/${analysisId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
  ...
  const getLoanRequestById = useCallback(
    async (requestId) => {
      ...
      try {
        ...
        throw new Error(
          response.data.message || "Failed to fetch loan request"
        );
      } catch (err) {
