...
const StudentEducationPlan = () => {
  ...
  const fetchAllData = async () => {
    try {
      const [planRes, kycRes, academicRes, admissionRes, workRes, coRes, profileRes] =
      ...
      if (planRes.status === "fulfilled") {
        ...
        if (plan) {
          setForm({
            ...
            livingExpenseOption: plan.livingExpenseOption || "WITH_LIVING_EXPENSE",
          });
        }
      }
  ...
  const handleSave = async () => {
    ...
    if (!form.targetCountry || !form.degreeType || !form.courseDurationMonths || !form.courseDetails || !form.loanAmountRequested) {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to save course details");
    } finally {
      setSaving(false);
    }
  };
  ...
  const DocumentCard = ({ label, filename, url }) => (
    <div className="space-y-1">
      ...
      {url && <p className="text-xs text-green-500">✓ Uploaded - Click to preview</p>}
    </div>
  );
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-5">
              ...
              <div className="mb-4">
                ...
                <ol className="text-sm text-gray-600 space-y-1 list-decimal list-inside">
                  ...
                  {academicDocs?.higherEducation?.length > 0 && <li>Graduation</li>}
              ...
              {/* Test Scores */}
              <div>
                <h4 className="font-medium text-gray-800 mb-2">Test Scores</h4>
                <ol className="text-sm text-gray-600 space-y-1 list-decimal list-inside">
                  <li>IELTS</li>
                  <li>GRE</li>
                  <li>TOEFL</li>
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-5">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-orange-500">Course Details</h2>
                {!editing ? (
                  <button
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    <label className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-sm text-gray-600">Bachelors (only secured loan)</span>
                    </label>
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    <label className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-sm text-gray-600">Yes, include living expenses</span>
                    </label>
                    <label className="flex items-center gap-2">
                      <input
                        type="radio"
                        ...
                        checked={form.livingExpenseOption === "WITHOUT_LIVING_EXPENSE"}
                        onChange={handleChange}
                        disabled={!editing}
                        className="w-4 h-4 text-orange-500 border-gray-300 focus:ring-orange-500"
                      />
                      <span className="text-sm text-gray-600">No, tuition fee only</span>
                    </label>
                  </div>
                </div>
            ...
            {/* Attached Documents */}
            <div className="bg-white rounded-lg border border-gray-200 p-5">
              <h2 className="text-xl font-semibold text-orange-500 mb-4">Attached Documents</h2>

              {/* Student KYC Documents */}
              <div className="mb-4">
                <h3 className="font-medium text-gray-800 mb-3">Student KYC Document</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <DocumentCard
                    label="Pan Card"
                    filename={kycDocs?.pancardUrl ? "pan_card.jpg" : "No file"}
                    url={kycDocs?.pancardUrl}
                  />
                  <DocumentCard
                    label="Aadhar Card"
                    filename={kycDocs?.aadhaarUrl ? "aadhar_card.jpg" : "No file"}
                ...
                <div className="mt-4">
                  <DocumentCard
                    label="Passport"
                    filename={kycDocs?.passportUrl ? "Passport.pdf" : "No file"}
              ...
              <div className="border-t border-gray-200 pt-4 mb-4">
                <h3 className="font-medium text-gray-800 mb-3">Academic Marksheets</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <DocumentCard
                    label="10th grade marksheet"
                    filename={academicDocs?.class10?.marksheetUrl ? "10th grade marksheet" : "No file"}
                    url={academicDocs?.class10?.marksheetUrl}
                  />
                  <DocumentCard
                    label="12th grade marksheet"
                    filename={academicDocs?.class12?.marksheetUrl ? "12th grade marksheet" : "No file"}
              ...
              <AccordionSection title="Secured Admissions" sectionKey="securedAdmissions">
                {admissionDocs ? (
                  <DocumentCard
                  ...
                  <p className="text-sm text-gray-500">No admission documents uploaded</p>
                )}
              </AccordionSection>

              <AccordionSection title="Work Experience" sectionKey="workExperience">
                  ...
                  <div className="space-y-2">
                    {workExperience.map((exp, idx) => (
                      <DocumentCard
                        ...
                        filename={exp.documentUrl ? `experience_${idx + 1}.pdf` : "No file"}
                        url={exp.documentUrl}
                      />
                    ))}
                  </div>
                ) : (
                  <p className="text-sm text-gray-500">No work experience documents uploaded</p>
                )}
              </AccordionSection>

              <AccordionSection title="Co-borrower Documents" sectionKey="coBorrowerDocuments">
                {coBorrowers.length > 0 ? (
                    ...
                    {coBorrowers.map((cb, idx) => (
                      <div key={idx} className="grid grid-cols-2 gap-4">
                        <DocumentCard
                          ...
                          filename={cb.panCardUrl ? "pan_card.pdf" : "No file"}
                          url={cb.panCardUrl}
                        />
                        <DocumentCard
                          label={`Co-borrower ${idx + 1} - Aadhaar`}
                          filename={cb.aadhaarCardUrl ? "aadhaar.pdf" : "No file"}
                  ...
                  <p className="text-sm text-gray-500">No co-borrower documents uploaded</p>
                )}
              </AccordionSection>
