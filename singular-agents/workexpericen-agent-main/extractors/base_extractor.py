...
class BaseExtractor:
    """Base class for work experience extraction with Gemini/OpenRouter"""
    ...
    def extract_structured(self,
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                print(f"   ⚠️ Request failed after {elapsed:.1f}s: {str(e)[:100]}")
                ...
                if any(x in error_str for x in ["timeout", "timed out", "time out"]):
                    ...
                    if attempt < self.max_retries - 1:
                        ...
                        print(f"   ⏰ Timeout occurred. Retrying with {new_timeout}s timeout...")
                        timeout = new_timeout
                        continue
                    else:
